 ...
 *
*/
package org.simccs.desktop.ui.experiment.create.controller;
...
import org.simccs.desktop.connectors.storage.GuiBulkFileUploadTask;
import org.simccs.desktop.connectors.storage.GuiFileDownloadTask;
import org.simccs.desktop.ui.commons.ImageButton;
import org.simccs.desktop.ui.commons.SimCCSDialogHelper;
import org.simccs.desktop.util.SimCCSContext;
import org.simccs.desktop.util.UserPrefs;
...
public class ExperimentCreateController {
    ...
    private String remoteDataDirRoot = SimCCSContext.getInstance().getGatewayUserDataRoot();
    ...
    private void initElements(){
        try{
            ...
            UserPrefs userPrefs = SimCCSContext.getInstance().getUserPrefs();
            ...
            expCreateAppField.valueProperty().addListener((observable, oldValue, newValue) -> {
                try {
                    ...
                    e.printStackTrace();
                    SimCCSDialogHelper.showExceptionDialogAndWait(e, "Exception Dialog", expCreateAppField.getScene().getWindow(),
            ...
            expSaveButton.setOnAction(event -> {
                try {
                    ...
                    SimCCSDialogHelper.showExceptionDialog(e,"Caught Exception",null, "Unable to create experiment");
                }
            });
            ...
            e.printStackTrace();
            SimCCSDialogHelper.showExceptionDialogAndWait(e, "Exception Dialog", expCreateNameField.getScene().getWindow(),
    ...
    public void initSingleCplexExperiment(String mpsInput) throws FileNotFoundException, TException, URISyntaxException {
        expCreateNameField.setText("SimCCS_job");
        expCreateNodeCountField.setText("1");
        expCreateTotalCoreCountField.setText("1");
//        String[] lines = mpsInput.split("\n");
...
//        }
        ...
        List<ApplicationInterfaceDescription> singleCplexApps = new ArrayList<>();
        for(ApplicationInterfaceDescription appInter : applicationInterfaceDescriptions){
            if(appInter.getApplicationName().toLowerCase().matches(SimCCSContext.getInstance().getSingleCplexAppName())
                    || appInter.getApplicationName().toLowerCase().matches(SimCCSContext.getInstance().getSingleCplexMPIAppName())){
                singleCplexApps.add(appInter);
            }
        }
        if(singleCplexApps.size() > 0){
            expCreateAppField.getItems().setAll(singleCplexApps);
            expCreateAppField.getSelectionModel().select(0);
            boolean parameterUpdateFlag=false;
            for(ApplicationInterfaceDescription app : singleCplexApps){
                List<InputDataObjectType> singleCplexInputs = app.getApplicationInputs();
                singleCplexInputs.get(0).setValue(mpsInput);
                if (!parameterUpdateFlag) {
                    updateExperimentInputs(singleCplexInputs, false);
    ...
    public void initEnsembleCplexExperiment(String mpsZipInput) throws FileNotFoundException, TException, URISyntaxException {
        expCreateNameField.setText("SimCCS_Ensemble_job");
        expCreateNodeCountField.setText("4");
        expCreateTotalCoreCountField.setText("32");
        expCreateWallTimeField.setText("60");
        List<ApplicationInterfaceDescription> applicationInterfaceDescriptions = AiravataManager.getInstance().getAllApplicationInterfaces();
        List<ApplicationInterfaceDescription> ensembleCplexApps = new ArrayList<>();
        for(ApplicationInterfaceDescription appInter : applicationInterfaceDescriptions){
            if(appInter.getApplicationName().toLowerCase().matches(SimCCSContext.getInstance().getEnsembleCplexAppName())
                    || appInter.getApplicationName().toLowerCase().matches(SimCCSContext.getInstance().getMultiCplexAppName())){
                ensembleCplexApps.add(appInter);
            }
        }
        //TODO: Will need to set the default parameter carefully after planned change in script on the Gateway server
        if(ensembleCplexApps.size() > 0){
            expCreateAppField.getItems().setAll(ensembleCplexApps);
            expCreateAppField.getSelectionModel().select(0);
            boolean parameterUpdateFlag=false;
            for(ApplicationInterfaceDescription app : ensembleCplexApps){
                List<InputDataObjectType> singleCplexInputs = app.getApplicationInputs();
                singleCplexInputs.get(0).setValue(mpsZipInput);
                if (!parameterUpdateFlag) {
                    updateExperimentInputs(singleCplexInputs, false);
                    parameterUpdateFlag=true;
                }
...
//                new FileChooser.ExtensionFilter("All Files", "*.*"));
        ...
        if(selectedApp != null){
            ...
            expCreateResourceField.valueProperty().addListener((observable, oldValue, newValue) -> {
                ...
                try{
                    ...
                    SimCCSDialogHelper.showExceptionDialog(ex, "Failed while retrieving user compute prefs", null, ex.getMessage());
                }
    ...
    private void loadAvailableBatchQueues(){
        ...
        if(selectedResource != null){
            ...
            expCreateQueueField.valueProperty().addListener((observable, oldValue, newValue) -> updateQueueSpecificLimitsInLabels());
    ...
    private void updateExperimentInputs(List<InputDataObjectType> inputDataObjectTypes, boolean hasOptionalFileInputs) throws TException, URISyntaxException {
        ...
        for(InputDataObjectType inputDataObjectType : inputDataObjectTypes){
            ...
            if(inputDataObjectType.getType().equals(DataType.STRING)){
                ...
                stringField.textProperty().addListener((observable, oldValue, newValue) -> experimentInputs.put(inputDataObjectType,newValue));
...
//                hBox.getChildren().add(1, remoteFilePickBtn);
                ...
                remoteFilePickBtn.setOnAction(event -> {
                    try {
                        ...
                        SimCCSDialogHelper.showExceptionDialogAndWait(e, "Exception Dialog", remoteFilePickBtn.getScene().getWindow(),
                ...
                if(selectedFile.exists()){
                    try {
                        ...
                        try {
                            ...
                            logger.error("Cannot open file. Opening parent directory");
                            SimCCSDialogHelper.showExceptionDialog(e, "Exception Dialog",
                                    expCreateInputsGridPane.getScene().getWindow(), "Failed Opening File");
                        }
                    }
                }else{
                    boolean result = SimCCSDialogHelper.showConfirmDialog("Confirm Action", "Remote File Download", "You have selected a remote file." +
...
//            hBox.getChildren().add(2, remoteFilePickBtn);
    ...
    private String showSelectRemoteFile() throws IOException {
        ...
        primaryStage.setTitle("SimCCS Desktop Client - Select Remote File");
    ...
    private void createExperiment(boolean launch) throws TException {
        if(validateExperimentFields()){
            ...
            String remoteDataDir = SimCCSContext.getInstance().getRemoteDataDirPrefix() + remoteDataDirRoot + randomString  + "/";
            ExperimentModel experimentModel = assembleExperiment(remoteDataDir, randomString + "/");
            Map<String,File> uploadFiles = new HashMap<>();
            for (Map.Entry<InputDataObjectType, Object> entry : experimentInputs.entrySet()) {
            ...
            if(uploadFiles.size() > 0){
                ...
                fileUploadService.setOnSucceeded(event -> createExperiment(experimentModel, launch));
    ...
    private void createExperiment(ExperimentModel experimentModel, boolean launch){
        try {
            if(!isEditExperiment) {
                ...
                SimCCSEventBus.getInstance().post(new SimCCSEvent(SimCCSEvent.SimCCSEventType.EXPERIMENT_CREATED
                        , experimentModel));
                ...
                SimCCSEventBus.getInstance().post(new SimCCSEvent(SimCCSEvent.SimCCSEventType.EXPERIMENT_UPDATED
                        ,experimentModel));
            }
            if(launch){
                try {
                    AiravataManager.getInstance().launchExperiment(experimentModel.getExperimentId());
                    SimCCSEventBus.getInstance().post(new SimCCSEvent(SimCCSEvent.SimCCSEventType.EXPERIMENT_LAUNCHED
                            ,experimentModel));
                }catch (TException e){
                    e.printStackTrace();
                    SimCCSDialogHelper.showExceptionDialogAndWait(e, "Exception Dialog", expSaveLaunchButton.getScene().getWindow(),
            ...
            e.printStackTrace();
            SimCCSDialogHelper.showExceptionDialogAndWait(e, "Exception Dialog", expSaveButton.getScene().getWindow(),
    ...
    private ExperimentModel assembleExperiment(String remoteDataDir, String experimentDataDir) throws TException {
        ...
        experimentModel.setGatewayId(SimCCSContext.getInstance().getAiravataGatewayId());
        experimentModel.setUserName(SimCCSContext.getInstance().getUserName());
        ...
        userConfigurationDataModel.setStorageId(SimCCSContext.getInstance().getGatewayaStorageId());
        ...
        for(InputDataObjectType inputDataObjectType : this.experimentInputs.keySet()){
            if(inputDataObjectType.getType().equals(DataType.URI)){
                ...
                if(((File) this.experimentInputs
                    ...
                    dpModel.setGatewayId(SimCCSContext.getInstance().getAiravataGatewayId());
                    dpModel.setOwnerName(SimCCSContext.getInstance().getUserName());
                    ...
                    rpModel.setStorageResourceId(SimCCSContext.getInstance().getGatewayaStorageId());
                ...
                if(files.get(0).exists()){
                    StringBuilder uriCollection = new StringBuilder();
                    for(File file : files){
                        if(file.exists()) {
                            ...
                            dpModel.addToReplicaLocations(rpModel);
                            String uri = AiravataManager.getInstance().registerDataProduct(dpModel);
                            uriCollection.append(uri).append(",");
                        }
                    }
                    uriCollection = new StringBuilder(uriCollection.substring(0, uriCollection.length() - 1));
                    inputDataObjectType.setValue(uriCollection.toString());
                }
            }else{
    ...
    private boolean validateExperimentFields(){
        ...
        if(expName == null || expName.isEmpty()){
            SimCCSDialogHelper.showWarningDialog("Warning Dialog", "Experiment Validation Failed", "Experiment name should" +
                    " be non empty");
            return false;
        }
        ...
        if(selectedQueue != null) {
            try {
                ...
                if (nodeCount <= 0 || nodeCount > selectedQueue.getMaxNodes()){
                    SimCCSDialogHelper.showWarningDialog("Warning Dialog", "Experiment Validation Failed", "Node count should" +
                ...
                if (coreCount <= 0 || coreCount > selectedQueue.getMaxProcessors()){
                    SimCCSDialogHelper.showWarningDialog("Warning Dialog", "Experiment Validation Failed", "Core count should" +
                ...
                if (wallTime <= 0 || wallTime > selectedQueue.getMaxRunTime()){
                    SimCCSDialogHelper.showWarningDialog("Warning Dialog", "Experiment Validation Failed", "Wall time should" +
        ...
        if(this.experimentInputs == null || this.experimentInputs.size()==0){
            SimCCSDialogHelper.showWarningDialog("Warning Dialog", "Experiment Validation Failed", "Experiment inputs" +
                    " not selected");
            return false;
        }else{
            for(InputDataObjectType inputDataObjectType : this.experimentInputs.keySet()){
                ...
                if(inputDataObjectType.isIsRequired() && (object == null || object.toString().equals(""))){
                    SimCCSDialogHelper.showWarningDialog("Warning Dialog", "Experiment Validation Failed", "No value" +
        ...
        Service<Boolean> service = new Service<Boolean>() {
            ...
            protected Task<Boolean> createTask() {
                try {
                    ...
                    e.printStackTrace();
                    SimCCSDialogHelper.showExceptionDialogAndWait(e, "Exception Dialog", expCreateInputsGridPane.getScene().getWindow(),
                            "Unable To Connect To File Server !");
                }
                return null;
            }
        };
        SimCCSDialogHelper.showProgressDialog(service,"Progress Dialog",expCreateInputsGridPane.getScene().getWindow(),
    ...
    private void downloadFile(Path remotePath, String destParentPath){
        ...
        SimCCSDialogHelper.showProgressDialog(service,"Progress Dialog",expCreateNameField.getScene().getWindow(),
                "Downloading File " + remotePath.getFileName());
        service.setOnFailed((WorkerStateEvent t) -> SimCCSDialogHelper.showExceptionDialogAndWait(service.getException(), "Exception Dialog",
                expCreateNameField.getScene().getWindow(), "File Download Failed"));
        service.setOnSucceeded((WorkerStateEvent t)-> SimCCSEventBus.getInstance().post(new SimCCSEvent(SimCCSEvent.SimCCSEventType.FILE_DOWNLOADED,localPath)));
        service.start();
    }
}