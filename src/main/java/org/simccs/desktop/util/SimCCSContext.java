 ...
 *
*/
package org.simccs.desktop.util;
...
public class SimCCSContext {
    private final static Logger logger = LoggerFactory.getLogger(SimCCSContext.class);

    private Map<String,String> dynamicConfigurations = new HashMap<>();

    private Properties properties = new Properties();
    private static final String PROPERTY_FILE_NAME = "/simccs.properties";

    private static SimCCSContext instance;

    private UserPrefs userPrefs;

    private SimCCSContext(){
        InputStream inputStream = SimCCSContext.class.getResourceAsStream(PROPERTY_FILE_NAME);
        try {
    ...
    public static SimCCSContext getInstance(){
        if(SimCCSContext.instance == null){
            SimCCSContext.instance = new SimCCSContext();
        }
        return SimCCSContext.instance;
    }
    public void reset(){
        SimCCSContext.instance = new SimCCSContext();
    }
    ...
    public String getFileDownloadLocation(){
        if(properties.getProperty(SimCCSConfig.DEFAULT_FILE_DOWNLOAD_PATH) != null && !properties.getProperty(
                SimCCSConfig.DEFAULT_FILE_DOWNLOAD_PATH).isEmpty()) {
            return properties.getProperty(SimCCSConfig.DEFAULT_FILE_DOWNLOAD_PATH);
        }else{
            return System.getProperty("user.home") + File.separator + "SimCCS" +  File.separator + "ExperimentData";
        }
    }

    public String getAiravataGatewayId(){
        if(SimCCSConfig.DEV){
            return properties.getProperty(SimCCSConfig.DEV_AIRAVATA_GATEWAY_ID);
        }
        return properties.getProperty(SimCCSConfig.AIRAVATA_GATEWAY_ID);
    }

    public void setUserName(String userName){ dynamicConfigurations.put(SimCCSConfig.USER_NAME, userName);}

    public String getUserName(){ return dynamicConfigurations.get(SimCCSConfig.USER_NAME);}
    ...
    public void setAuthenticated(boolean authenticated) {
        if(authenticated)
            dynamicConfigurations.put(SimCCSConfig.AUTHENTICATED, "true");
        else
            dynamicConfigurations.remove(SimCCSConfig.AUTHENTICATED);
    }

    public boolean getAuthenticated() { return dynamicConfigurations.containsKey(SimCCSConfig.AUTHENTICATED); }

    public void setOAuthToken(String oauthToken) {
        dynamicConfigurations.put(SimCCSConfig.OAUTH_TOKEN,oauthToken);
    }

    public String getOAuthToken(){
        return dynamicConfigurations.get(SimCCSConfig.OAUTH_TOKEN);
    }

    public void setRefreshToken(String refreshToken){
        dynamicConfigurations.put(SimCCSConfig.OAUTH_REFRESH_TOKEN, refreshToken);
    }

    public String getRefreshToken(){
        return dynamicConfigurations.get(SimCCSConfig.OAUTH_REFRESH_TOKEN);
    }

    public void setTokenExpiaryTime(long tokenExpiarationTime) {
        dynamicConfigurations.put(SimCCSConfig.OAUTH_TOKEN_EXPIRATION_TIME, tokenExpiarationTime + "");
    }

    public long getOAuthTokenExpirationTime(){
        return Long.parseLong(dynamicConfigurations.get(SimCCSConfig.OAUTH_TOKEN_EXPIRATION_TIME));
    }

    public String getAiravataHost() {
        if(SimCCSConfig.DEV){
            return properties.getProperty(SimCCSConfig.DEV_AIRAVATA_HOST);
        }
        return properties.getProperty(SimCCSConfig.AIRAVATA_HOST);
    }

    public int getAiravataPort() {
        if(SimCCSConfig.DEV){
            return Integer.parseInt(properties.getProperty(SimCCSConfig.DEV_AIRAVATA_PORT));
        }
        return Integer.parseInt(properties.getProperty(SimCCSConfig.AIRAVATA_PORT));
    }

    public String getSFTPHost() {
        if(SimCCSConfig.DEV){
            return properties.getProperty(SimCCSConfig.DEV_SFTP_HOST);
        }
        return properties.getProperty(SimCCSConfig.SFTP_HOST);
    }

    public int getSFTPPort() {
        if(SimCCSConfig.DEV){
            return Integer.parseInt(properties.getProperty(SimCCSConfig.DEV_SFTP_PORT));
        }
        return Integer.parseInt(properties.getProperty(SimCCSConfig.SFTP_PORT));
    }

    public String getSingleCplexAppName() {
        return "cplex";
    }

    public String getSingleCplexMPIAppName() {
        return "cplex_mpi";
    }

    public String getMultiCplexAppName() {
        return "cplex_multi";
    }

    public String getEnsembleCplexAppName() {
        return "cplex_ensemble";
    }

    public String getGatewayaStorageId(){
        if(SimCCSConfig.DEV){
            return properties.getProperty(SimCCSConfig.DEV_GATEWAY_STORAGE_ID);
        }
        return properties.getProperty(SimCCSConfig.GATEWAY_STORAGE_ID);
    }

    public String getGatewayUserDataRoot(){
        if(SimCCSConfig.DEV){
            return properties.getProperty(SimCCSConfig.DEV_REMOTE_DATA_DIR_ROOT) + getUserName() + "/";
        }
        return properties.getProperty(SimCCSConfig.REMOTE_DATA_DIR_ROOT) + getUserName() + "/";
    }

    public String getRemoteDataDirPrefix(){
        if(SimCCSConfig.DEV){
            return properties.getProperty(SimCCSConfig.DEV_REMOTE_DATA_DIR_PREFIX);
        }
        return properties.getProperty(SimCCSConfig.REMOTE_DATA_DIR_PREFIX);
    }
