...
import org.simccs.desktop.ui.login.LoginWindow;
...
public class SimCCSDesktop extends Application{
    private static Logger logger;

    public SimCCSDesktop(){
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        if(dataDir.exists()) {
            ...
            SimCCSDialogHelper.showExceptionDialogAndWait(new Exception("Application Data Dir Does Not Exists"),
    ...
    @Subscribe
    public void handleSimCCSEvents(SimCCSEvent event){
        if(event.getEventType().equals(SimCCSEvent.SimCCSEventType.LOGOUT)){
            try {
                start(new Stage());
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static void initApplicationDirs() throws IOException {
        ...
        if(!appDataRoot.canWrite()){
            SimCCSDialogHelper.showExceptionDialogAndWait(new Exception("Cannot Write to Application Data Dir"),
        ...
        logger = LoggerFactory.getLogger(SimCCSDesktop.class);
        File logParent = new File(applicationDataDir() + "logs");
        if(!logParent.exists())
            logParent.mkdirs();
        PrintStream outPs = new PrintStream(applicationDataDir() + "logs/simccs.std.out");
        PrintStream errPs = new PrintStream(applicationDataDir() + "logs/simccs.std.err");
        System.setOut(outPs);
        System.setErr(errPs);
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            outPs.close();
            errPs.close();
            SimCCSContext.getInstance().saveUserPrefs();
        }));
...
//        extractLegacyEditorResources();
    ...
    public static void createTrustStoreFileIfNotExists() throws IOException {
        ...
        if(!targetFile.exists()) {
            InputStream initialStream = SimCCSContext.class.getResourceAsStream("/client_truststore.jks");
    ...
    /*public static void extractLegacyEditorResources() {
        try {
            ...
            if(!appHome.exists()){
                if(!appHome.mkdirs()){
                    SimCCSDialogHelper.showExceptionDialogAndWait(new Exception("Cannot Create Application Data Dir"),
            ...
            ZipEntry zipentry;
            zipinputstream = new ZipInputStream(SimCCSDesktop.class.getClassLoader().getResourceAsStream("legacy.editors.zip"));

            zipentry = zipinputstream.getNextEntry();
            if(zipentry == null){
                SimCCSDialogHelper.showExceptionDialogAndWait(new Exception("Cannot Read Application Resources"),
    ...
    {
        return System.getProperty("user.home") + File.separator + "SimCCS" + File.separator;
    }
}