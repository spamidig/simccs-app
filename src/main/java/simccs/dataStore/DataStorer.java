...
public class DataStorer {
    ...
    private Sink[] sinks;
    private Source[] activeSources;
    private Sink[] activeSinks;
    private LinearComponent[] linearComponents;
    private int[] sourceSinkCellLocations;  // Cell number for each source and sink node

    // Uncertainty data.
    private SinkUncertainty[] sinkUncertainties;
    private SourceUncertainty[] sourceUncertainties;
...
//        }
    ...
    public void generateShortestPaths() {
        loadNetworkCosts();
        setAdjacencyCosts(adjacencyCosts);
...
//        if (sourceSinkCellLocations == null) {
        ArrayList<Source> activeSources = new ArrayList<>();
        ArrayList<Sink>activeSinks = new ArrayList<>();
        for (Source source : sources) {
        ...
        this.activeSources = activeSources.toArray(new Source[0]);
        this.activeSinks = activeSinks.toArray(new Sink[0]);

        sourceSinkCellLocations = new int[activeSources.size() + activeSinks.size()];
        for (int i = 0; i < activeSources.size(); i++) {
            sourceSinkCellLocations[i] = activeSources.get(i).getCellNum();
        }
        for (int i = 0; i < activeSinks.size(); i++) {
            sourceSinkCellLocations[activeSources.size() + i] = activeSinks.get(i).getCellNum();
        }
//        }
    ...
    public Sink[] getActiveSinks() {
        return activeSinks;
    }

    public SinkUncertainty[] getSinkUncertainties() {
        return sinkUncertainties;
    }

    public SourceUncertainty[] getSourceUncertainties() {
        return sourceUncertainties;
    }
    ...
    public void setSinkUncertainties(SinkUncertainty[] sinkUncertainties) {
        this.sinkUncertainties = sinkUncertainties;
    }

    public void setSourceUncertainties(SourceUncertainty[] sourceUncertainties) {
        this.sourceUncertainties = sourceUncertainties;
    }
